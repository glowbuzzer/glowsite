...
<StandardCarousel width={"500px"} height={"120px"} fade={true} labelsOn={false} dotsTop={'480px'} dotsOn={false} pauseOnHover={false} imgdata={[
...
### As a master
As part of the &gbt we supply an EtherCAT master (&gbem) which can run either on Linux or a microcontroller. On Linux this talks to the &gbc via a shared memory instance using Linux signals to synchronise the cycle of &gbc to the distributed clock on the EtherCAT network.
