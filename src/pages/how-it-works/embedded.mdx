...
export const dataSourceEmbeddedLinux = [
    ...
    {
        key: '4',
        pros: "Ease e.g. spin up a websockets server on Linux = five minute job where as on a microcontroller it is a bit of mission, filesystem",
...
And this gives you the ability to build complex co-ordinated motion control applications (robot, Cartesian, delta etc.) built using web stack and running on a fully embedded platform with no PLC in the loop!
...
The &gbtt needs a set of uC middleware to operate:
* RTOS - e.g. freertos
* FS - e.g. littleFs
* Network - e.g. LWIP (the toolkit includes the embedded websockets, ftp  and http servers)
* Screen - e.g.
...
* M7
    ...
    * Trajectory planner
    * SPI master to talk to hardware slave controller chips or to the &gbem or to EtherCAT slave chips (e.g. LAN9252) or mult-protocol slave chips (e.g. Netx 51)
...
1. Drives are external to the board, and you communicate with them over industrial ethernet (feldbus). E.g. the huge range of EtherCAT drives from the likes of Kollmorgen, Omron, Beckhoff, Panasonic etc.
...
If you are using STM32, the recommended development environment just uses the standard ST development boards. The Nucleo for developing the EtherCAT or Step Master (&gbem / &gbsm) and the Discovery board for developing &gbc (real-time control).

<Row>
    <Col span={12}>
        <Image meta={nucleo} alt="STM32 H7 Nucleo" preset="narrow"/>
        <h3>STM32 H7 Nucleo dev. board</h3>
    </Col>
    <Col span={12}>
         <Image meta={disco} alt="STM32 H7 Discovery" preset="narrow"/>
