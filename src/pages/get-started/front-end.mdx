...
## Introduction
This document takes you though installing the &gbt React components and creating some simple machine control functions of your own.
...
useDigitalOutput is the &gbt component that integrates with the Digital IO.
...
The `useDigitalOutput` component from the &gbt has the following signature:
...
For this tutorial we will use digital output number 1.
...
And finally connect the &gbr tiles to &gbc.

<FrontendConnect/>

We will be running in `Simulation Mode` as we have not integrated &gbc with any hardware (e.g. an EtherCAT master).

Now, in the UI, the &gb provided Digital Outputs tile reflects the state of all the digital outputs and you will be able to see Digital Output 1 toggle as your move your switch.
