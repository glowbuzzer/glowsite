...
### **Tiles** - you can add pre-built tiles like `JogCartesianTile`, `GCodeTile` etc. to your React project which give you an "out of the box" way to invoke motion primitives.

The `JogJointsTile` & `JogCartesian` allows you to move Joints and Kinematics Configurations either to a specific position or be "jogged" manually to a position. See <GlowsiteLink to="/docs/gbr/tiles/JogJointsTile">JogTile</GlowsiteLink> &  <GlowsiteLink to="/docs/gbr/tiles/JogCartesianTile">JogCartesianTile</GlowsiteLink>for more information.
...
For example for a `MoveToPosition` activity you would start by creating a <GlowsiteLink to="/docs/types/PointsConfig">PointsConfig</GlowsiteLink> item for the positions  you want to move to.

Then, in the <GlowsiteLink to="/docs/types/MoveToPosition">MoveLineConfig</GlowsiteLink> section of your config file you would reference the index of the line object you created together with the index of the kinematicsConfiguration you want to move and the index of the moveParameters (velocity of the move etc.) that you want to use.
