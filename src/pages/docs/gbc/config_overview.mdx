...
&gbc is configured with a JSON file read at start-up using the `./GBC -cCONFIG_FILE_NAME` command line argument.

If a config file is not supplied to &gbc on the command line, &gbc uses a default configuration "baked into" the executable.

The configuration files covers different areas of the machine and &gbt configuration which are described below.

## Config file sections

See <Link to="/docs/types/ConfigType">Config</Link> or an overview of the contents of the config.

### `machine: {}` configuration and parameter definition
See <Link to="/docs/gbc/configuration/config_machine">Machine config</Link> for details.

### `kinematicsConfiguration: {}, joint: {}` configuration and parameter definition
See <Link to="/docs/gbc/configuration/config_joints_and_kinematics">Joint and kinematics config</Link> for details.

### `dout: {}, din: {}, din: {}, din: {}, aout: {}, ain: {}, ain: {}, iout: {}, iin: {}` configuration and parameter definition
See <Link to="/docs/gbc/configuration/config_io">IO config</Link> for details.

### `tool: {}` configuration and parameter definition
See <Link to="/docs/gbc/configuration/config_tool">Tool config</Link> for details.

### `spindle: {}` configuration and parameter definition
See <Link to="/docs/gbc/configuration/config_spindle">Spindle config</Link> for details.

### `stream: {}, soloActivity: {}` configuration and parameter definition
See <Link to="/docs/gbc/configuration/config_streams_and_activities">Streams and Activities config</Link> for details.

### `fieldbus: {}` configuration and parameter definition
See <Link to="/docs/gbc/configuration/config_fieldbus">Fieldbus config</Link> for details.

### `moveParameters: {}, task: {}, activity: {}, lines: {}, arcs: {}, frames: {}, cartesianPositions: {}, triggerOn: {}` configuration and parameter definition
See <Link to="/docs/gbc/configuration/config_advanced">Advanced config</Link> for details.
