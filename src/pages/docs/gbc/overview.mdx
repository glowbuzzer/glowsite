...
<Tldr>
    ...
    - POSIX sharedmemory or SPI interface to downstream
...
The platform GBC runs on will need al least two Ethernet/Wi-Fi ports if GBC is being combined with GBEM (EtherCAT master). One for websockets and one for EtherCAT (two needed for cable redundant EtherCAT).
