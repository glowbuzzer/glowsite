...
<Tldr>
    - Front-end - 100ms
    - JS inside GBC - 10ms
    - GBEM/GBSM - bus cycle time
</Tldr>

GBC allows user provided code to be run in different tiers within the application. The response time between trigger events occurring and the code being executed varies by the tier the code is deployed into.
...
* GBEM/GBSM -  use defined c code "PLC style" functions can be deployed to the downstream process controlling communication with the fieldbus / motor drivers. These functions can respond to an event within the bus cycle time (1ms, 2ms, 4ms etc.) in a low jitter manner. These have to be compiled into the executable code.
