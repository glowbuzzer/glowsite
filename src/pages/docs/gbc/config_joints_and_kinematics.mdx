...
In the config there is a parameter `"finiteContinuous"`. This is the enum `JOINT_FINITECONTINUOUS`. <GlowsiteLink to="/docs/types/JOINT_FINITECONTINUOUS">See here for more details</GlowsiteLink>
...
The `"jointType"` is the enum `JOINT_TYPE` with 0 = `JOINT_PRISMATIC`= Joint type is prismatic (linear) and 1 = `JOINT_REVOLUTE` Joint type is revolute (rotary). <GlowsiteLink to="/docs/types/JOINT_TYPE">See here for more details</GlowsiteLink>
...
`"kinematicsConfigurationType"` is the enum `KC_KINEMATICSCONFIGURATIONTYPE` which defines the mathematical kinematics model used for the kc <GlowsiteLink to="/docs/types/KC_KINEMATICSCONFIGURATIONTYPE">see here for more details</GlowsiteLink>
