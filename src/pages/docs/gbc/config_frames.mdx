...
N.B. `"absRel"` is the `FRAME_ABSRELATIVE` enum and 0 = `FRAME_ABSOLUTE` which means the frame is referenced with absolute position and the parent is ignored. <Link to="/docs/gbc/schema/FRAME_ABSRELATIVE">See here for more details</Link>
...
<Card title="frames Schema docs" extra={<Link to="/docs/gbc/schema/FramesConfig">docs Link</Link>} style={{ width: 300 }}>
