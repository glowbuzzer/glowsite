...
N.B. `"absRel"` is the `FRAME_ABSRELATIVE` enum and 0 = `FRAME_ABSOLUTE` which means the frame is referenced with absolute position and the parent is ignored. <GlowsiteLink to="/docs/types/FRAME_ABSRELATIVE">See here for more details</GlowsiteLink>
