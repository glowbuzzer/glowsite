...
The GBC and the front-end React components in the glowbuzzer toolkit provide support for:
...
This would likely take the form (for input) of an isolated, single or multi-channel ADC that provides an SPI interface that would be integrated with the embedded Linux or micrcontroller running GBCSM.
