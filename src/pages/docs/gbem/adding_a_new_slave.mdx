...
# {props.subtitle}
<Tldr>
    - Adding a new type of slave require a set of #defines
    - For more complex slaves c functions may need to be written
    - Many examples of different types of slaves provided
    - A one-off exercise that you do to support any non-standard slaves used by your machine
</Tldr>
...
2. #defines for the slave's EEPROM information - for example, `EEP_NAME`
3. #defines for BIT numbers used by the slave - for example, `HOMING_START_BIT_NUM`
4. #defines for indices of PDO objects - for example, `SETPOS_PDO_DRIVE_INDEX`
...
If the slave is a drive, then more functions will be needed - see the <Link to="/docs/gbem/adding_a_new_drive">Adding a new drive</Link> documentation.
...
See the examples below and the <Link to="/docs/gbem/adding_a_new_drive">Adding a new drive</Link> for more information on how to implement this function.
...
These are usually used for drives - see <Link to="/docs/gbem/adding_a_new_drive">Adding a new drive</Link> for more details of their use in the context of drives.
...
### Medium difficulty example - Beckhoff EL3102 slave

The EL3102 is a  2-channel analog input, voltage, ±10 V, 16 bit, differential.

The first bit of the driver code for the EL3102 is the #defines for the EEPROM information. These can be read off the netscan's output.
...
Next we create the #defines for the locations of the data in the slave's PDO.

Again, we refer to the `netscan` output.
...
PDO mapping according to CoE :
  ...
  SM3 inputs
 	[byte_offset] [addr b  ] [index: sub ] [bitl] [data_type   ] [name                                    ]
	[0          ] [0xFFF68818.0] [0x3101:0x01] [0x08] [UNSIGNED8   ] [Status                                  ]
	[1          ] [0xFFF68819.0] [0x3101:0x02] [0x10] [INTEGER16   ] [Value                                   ]
	[3          ] [0xFFF6881B.0] [0x3102:0x01] [0x08] [UNSIGNED8   ] [Status                                  ]
	[4          ] [0xFFF6881C.0] [0x3102:0x02] [0x10] [INTEGER16   ] [Value                                   ]

```

We can see from the `netscan` that the slave has no outputs only inputs.

The objects are a pair of `Status` and `Value` objects. The `Status` is an `uint8` and the `Value` is an `int16`.

This exactly what we would expect for a dual channel 16 bit differential voltage input slave.

The code to support the slave will just be a header file containing the defines.
...
You can refine this a little further with some #defines to interpret the status bits
...
PDO mapping according to CoE :
  SM2 outputs
        ...
        [4          ] [0xFFF84EAC.0] 	[0x6040:0x00] [0x10] // CANopen controlword UINT16
...
We can quickly see that the PDO mapping has been successfully applied. We can further confirm this by looking up the objects detailed in the netscan output, for example, `0x607A` in the Kollmorgen manual and we can confirm it is indeed "Target position INT32". The comments in the above output have been added by looking up the hex values in the Kollmorgen manual.
