...
# Compiling & Running GBEM
<Tldr>
    - Uses cmake for Linux build
    - Need to get the code in your IDE
    - Separate build for Linux vs. Raspberry Pi
</Tldr>

## Compiling
On Linux, there is a single executable, GBEM, which is run with a number of command line options.

This must be compiled before you can run it using the source code from GitHub.

Clone with: `git clone https://github.com/glowbuzzer/gbem`

Assuming you are doing a local compilation then to compile and run, outside of an IDE, the steps are:
