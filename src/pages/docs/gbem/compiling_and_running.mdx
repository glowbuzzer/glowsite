...
1. interface - `-i` - interface to use - this is a NIC interface, e.g. eth0
...
When you want to scan the EtherCAT network you will run &gbem with: `./GBEM -n -ieth0`.
...
* Have configured a machine see <Link to="/docs/gbem/configuring_machines">Configuring machines</Link>

Start out by doing a netscan. This is `./gbem -n -ieth0`.
...
If running on Linux platforms that use an SD card or eMMC as its filesystem, and you want to use emstat (<Link to="/docs/gbem/emstat">docs here</Link>) to write a JSON file each containing that status of the &gbem then it is useful to store files in RAM based storage rather than on storage that may have limited write/cycles.
