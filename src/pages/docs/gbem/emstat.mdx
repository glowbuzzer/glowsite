...
# {props.subtitle}
<Tldr>
    - &gbem writes a pair of JSON files containing the current machine state periodically to the filesystem
    - Example React code provided to give a UI to view these files
    - A "starter" for  you to adapt to write your own custom UI
</Tldr>

## Introduction

emstat provides a "live" view of what is going on inside of &gbeml.

&gbem writes a json file to the filesystem periodically (every couple of seconds). This status file contains:
...
emstat produces a file every n secs when n is defined by `JSON_STATUS_UPDATE_PERIOD_SECS`

emstat is enabled with the define `ENABLE_EMSTAT`

This emstat JSON files are served by the &gbc HTTP server.
