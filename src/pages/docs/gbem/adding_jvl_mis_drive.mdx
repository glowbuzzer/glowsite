...
# Adding support for JVL MIS drive - a blow-by-blow account

<Tldr>
    - Integrate JVL MIS drive with the toolkit
    - Use MacTalk to test drive
    - Scan drive with GBEM
    - Write drive interface functions
    - Handle 32-bit reference clocks (DC)
</Tldr>
...
import jvl_mis_drive from "./jvl_mis_drive.png?glowsite"

<Image meta={jvl_mis_drive} alt="JVL MIS Drive" maxWidth={400} />
...
import jvl_m12_rj45 from "./jvl_m12_rj45.png?glowsite"

<Image meta={jvl_m12_rj45} alt="JVL M12 RJ45 connector" maxWidth={400} />
...
<BogStandardHeaderRow>
    <BogStandardCol span={4}>
        Part Number
    </BogStandardCol>
    <BogStandardCol span={4}>
        Short description
    </BogStandardCol>
    <BogStandardCol span={16}>
        Long Description
    </BogStandardCol>
</BogStandardHeaderRow>
...
<BogStandardRow>
    ...
    <BogStandardCol span={16}>
        Int.Step 12-72VDC, EtherCAT, AbsC-L, Serie Nr.: 250690, 4xM12. 2x4pF:EtherCAT 17pF:8xDIO/AI+RS422+RS485, High Resolution: 409.600 step/rev. ±0.01 RPM, Programmable (incl. current, position & velocity), C-L + Abs Multiturn Enc. 4096/1024 CPR ± 5245, Revs, 8x24 w/ 3x10 KeyShaft: IP42 Motor: IP42, 43.1x106.0 mm Holding Torque: 0.80 Nm Radial, Connector 12-72 VDC Mechanical Power: 134 W
    </BogStandardCol>
</BogStandardRow>
...
<BogStandardRow>
    ...
    <BogStandardCol span={16}>
        RS485 M12-17pin DSub, 5m v1.1, For ServoStep MIS17x...43x + SMC66/85 w/Ethernet, and MAC motor® w/Ethernet, MAC00-Ex41
    </BogStandardCol>
</BogStandardRow>
...
<BogStandardRow>
    ...
    <BogStandardCol span={16}>
        M12 Shl Cable 5m 5 pin Fem 0°, Power
    </BogStandardCol>
</BogStandardRow>

<BogStandardRow>
    <BogStandardCol span={4}>
        WI1046-M12M4T05TRJ45
    </BogStandardCol>
    <BogStandardCol span={4}>
        Ethernet cable
    </BogStandardCol>
    <BogStandardCol span={16}>
        M12 Cab D-code Ethernet M0°-RJ45, 5m, Ø5
    </BogStandardCol>
</BogStandardRow>
...
<BogStandardRow>
    ...
    <BogStandardCol span={16}>
        USB to RS485 non-isolated adapter. 0,5m
    </BogStandardCol>
</BogStandardRow>
...
<BogStandardRow>
    ...
    <BogStandardCol span={16}>
        MAC/MIS/SMC Win software+manuals USB8G
    </BogStandardCol>
</BogStandardRow>
...
import jvl_unboxing from "./jvl_unboxing.jpg?glowsite"

<Image meta={jvl_unboxing} alt="JVL MIS unboxing" maxWidth={400} />
...
The power supply connector wiring colours (using the JVL manufactured power cable) are as follows
...
For production use and use with multiple drives you are going to need to have separate PSUs for motor and logic power and you need to consult the JVL manual for details of the power supply characteristics.
...
import jvl_power_connector_prep from "./jvl_power_connector_prep.jpg?glowsite"

<Image meta={jvl_power_connector_prep} alt="JVL power cable prep"  maxWidth={400} />
...
import jvl_scan from "./jvl_scan.png?glowsite"

<Image meta={jvl_scan} alt="JVL scan" preset="wide" />
...
import jvl_ethercat from "./jvl_ethercat.png?glowsite"

<Image meta={jvl_ethercat} alt="JVL MacTalk EtherCAT" preset="wide" />
...
The next steps cover how to the write the driver code. If you want to spoil all the fun and jump to the end and just look at the code it is available here:

<GitHubLink title={"Drive interface code"} repo={"gbem"} directory={"/libs/JVL"}/>
...
Then we run `gbem -n -ieth0`. And this should produce the following output:
...
So with this drive we can't assign additional PDO objects to SM2 and SM3, but we can insert additional objects to the PDO content held at 0x1600 and 0x1a00.
...
- 0x6061:0/8 bits - display modes of operation (MOO DISP)
 ...
 - 0x6077:0/16 bits - torque actual value (actAcc)
...
We will not need a `MAP_DRIVE_HOMING_EXEC_FUNCTIONS` as our drive has a nifty absolute multiturn encoder, so we are spared the pain of using the on-drive switch based homing functions.
...
Now we can come back to a drive header file, `jvl_mis.h`, and here we need to add our variable definitions for the PDO mapping objects:
...
As we only have a single slave on our EtherCAT network (the drive) we can't define an estop digital input, so we need to disable this in `gbem_config.h` with the #define:

```c
#define DISABLE_ESTOP_CHECKING                          1
```

Now we can run netscan again, this time with the `-m` option - `gbem -m -ieth0`. The -m option runs the PDO mapping function we just wrote before doing the scan of the network. If you have the wind on your back, you should see:
...
import jvl_dc_sync_ok from "./jvl_dc_sync_ok.png?glowsite"

<Image meta={jvl_dc_sync_ok} alt="JVL sync ok" preset={"wide"} />
...
During GBEM boot you may have noticed a warning from the function `check_dc_clock_type` saying the JVL drive only supported a 32 bit DC reference clock. A 32 bit register holding a nanoseconds time overflows in 4-or-so seconds and this is causing the juddering issue.
...
In the CMakeList.txt for machine_mapping add the JVL to the target_link_libraries() section


<YoutubeEmbed embedId="Z3Ixc7kfLDM" />
