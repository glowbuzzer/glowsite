...
In &gbem adding a new drive a set of extra steps on top of those required to add a new slave, so it is worth reviewing the documentation for adding a new slave as well as the instructions in this section - <Link to="/docs/gbem/adding_a_new_slave">Adding a new slave</Link>.
...
1. Dive into the manufacturer's drive documentation to work out the PDO mapping you want to use, [see here](#Write_slave_functions).
1. Dive into the manufacturer's docs to work out the start-up SDOs and any initial PDO writes that are needed.
1. Write the functions to perform the initial SDO writes, [see here](#Initial_sdos) .
1. Write the cyclic functions to communicate with the drives PDOs, [see here](#Cyclic_functions).
1. Test the drive, [see here](#Test_drive).
...
To create the PDO mapping see the section in the slave document <Link to="/docs/gbem/adding_a_new_slave">Adding a new slave</Link>
...
See the section in the slave document <Link to="/docs/gbem/adding_a_new_slave">Adding a new slave</Link> for more information on how to write the `ec_pdo_map_mydrive` function.
