...
import {BogStandardCol, BogStandardHeaderRow, BogStandardRow} from "../../../framework/components";
import {GlowsiteLink} from "../../../framework";

# {props.subtitle}

<Tldr>
    - To add support for a new type of drive, you need to write a set of c functions
    - If your drive follows the the CiA 402 spec. it is trivial
    - Lots of examples are provided to copy-and-paste from
    - If you are using a pre-supported drive you don't need to do this
</Tldr>
...
In &gbem adding a new drive a set of extra steps on top of those required to add a new slave, so it is worth reviewing the documentation for adding a new slave as well as the instructions in this section - [Adding a new slave](/docs/gbem/adding_a_new_slave).
...
1. Dive into the manufacturer's drive documentation to work out the PDO mapping you want to use, [Write slave functions](#Write_slave_functions).
1. Dive into the manufacturer's docs to work out the start-up SDOs and any initial PDO writes that are needed.
1. Write the functions to perform the initial SDO writes, Configuring initials SDO writes(#Initial_sdos) .
1. Write the cyclic functions to communicate with the drives PDOs, [Cyclic functions](#Cyclic_functions).
1. Test the drive, [Drive testing](#Test_drive).
...
SDO read/write function names follow the pattern `ec_sdo_write_xxxx` & `ec_sdo_read_xxxx` where `xxxx` is the type for example, `uint16`.

PDO get/set function names follow the pattern `ec_pdo_set_output_xxxx` & `ec_pdo_get_input_xxxx` where `xxxx` is the type for example, `uint16`.
...
<BogStandardHeaderRow>
    <BogStandardCol span={8}>Byte 0</BogStandardCol>
    <BogStandardCol span={8}>Byte 1:3 (multiplexer)</BogStandardCol>
    <BogStandardCol span={8}>Byte 4:7 (data)</BogStandardCol>
</BogStandardHeaderRow>
<BogStandardRow>
    <BogStandardCol span={8}>Command specifier  (upload/download etc.)</BogStandardCol>
    <BogStandardCol span={8}>16 bit index + 8 bit sub-index</BogStandardCol>
    <BogStandardCol span={8}>1-4 byte parameter data</BogStandardCol>
</BogStandardRow>
...
The slave number is an integer that represents the position of the slave on the network (note, index starts at 1). It is also usually a #define in the slave's header file.
...
1. *slave functions* which configure the slave itself (for example, configuration of the slave's PDO map)
2. *drive functions* most of which are called cyclically to exchange data with the drive each cycle (i.e. write a set position to a drive)
...
1. Take the default that the drive manufacturer has decided the drive will start-up with
...
Which option you do pick? There are some mandatory objects &gbem needs mapping. These are the core CiA 402 objects:
...
To create the PDO mapping see the section in the slave document [Adding a new slave](/docs/gbem/adding_a_new_slave).

If you are using the drive's default PDO mapping this function won't be needed and its entry in the machine map will be NULL.

If you want (or have) to create PDO map using the drive then this function will need to be written. 

See the section in the slave document [Adding a new slave](/docs/gbem/adding_a_new_slave) for more information on how to write the `ec_pdo_map_mydrive` function.
...
First it performs a series of SDO writes using the `ec_sdo_write_xxxx` functions from `ethercatgetset.c`. The #defines for the addresses (`xxxx_SDO_INDEX` and `xxxx_SDO_SUB_INDEX`) have come from the drive's reference manual and have been stored in the `mydrive.h` file.
...
There is a set of #defines in cia402.h for the standard values (for example, `CIA_MOO_CSP`) that we can use in this function. Usually the Modes of Operation will be an uint8_t or int8_t and so if we are reading a PDO value we will use one of the `ethercatgetset` functions,  most likely `ec_pdo_get_input_uint8`.
...
* **Drive provides the actual follow error value in a PDO** - the function will need to read the PDO with for example an `ec_pdo_get_input_int32`function and then threshold the returned value. If the follow error is greater than XXXX then return true and if less than return false
...
This function is called when an error occurs. What we want to return from this function is a string that describes what is wrong with the drive, for example, "Motor overheat".
...
Once the drive functions have been written they need to be linked into a machine map. See the documentation [CONFIGURING MACHINES](/docs/gbem/configuring_machines).
...
1. Use the drive's own in built-in homing functions where the limit switches are connected to the drives own inputs (this is recommended)
