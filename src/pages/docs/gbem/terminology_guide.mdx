...
* iomap - the mapping between EtherCAT slave IO and &gbc and PLC variables. For example, a digital input on a PDO (for example, channel 1 on an EL1008 module) can be mapped to a &gbc IO so that code running the the front-end can act on its status
* machine - collection of drives and IO that make up a machine
* drive - the electronics that control a motor
...
* Limits - soft limits - position limit - constraints on the envelope of movement of a drive. Moving outside the envelope will usually cause a drive to stop and error.
* SetPos, SetVel, SetAcc/SetTorque - the value given to a drive to set (command) its position/velocity/torque
* ActPos, ActVel, ActAcc/ActTorque - the actual value of a drive's position/velocity/torque
* Feedback in the context of drives means an encoder either built into a motor or external to the motor mounted on the moving part of the machine used by the drive electronics
* Incremental encode - an encoder that provides the change in position but not the absolute position itself
* Absolute encoder - an encoder that provides absolute position information this means the position is retained regardless of whether the encoder is powered or not, even if a movement is made without power to the encoder and power is reapplied, the encoder knows it true new position
* Multi-turn encoder - an encoder that provides position information spanning more than one turn (i.e. the total number of turns made plus the position within the current turn)
