...
export function fk_tx40(thetas: number[]): {
        ...
        List(
            ...
            -(Cos(theta5) * (-(Cos(theta2) * Cos(theta3)) + Sin(theta2) * Sin(theta3))) +
                Cos(theta4) *
                    ...
                    Sin(theta5),
            -225 * Sin(theta2) +
                225 * (Cos(theta2) * Cos(theta3) - Sin(theta2) * Sin(theta3)) +
                65 *
                    (-(Cos(theta5) * (-(Cos(theta2) * Cos(theta3)) + Sin(theta2) * Sin(theta3))) +
                        Cos(theta4) *
                            (-(Cos(theta3) * Sin(theta2)) - Cos(theta2) * Sin(theta3)) *
...
export function ik_tx40(
    ...
    const theta1 = () => {
        ...
        const sqrt = wx * wx + wy * wy - d3 * d3
        const theta1c = Math.sqrt(Math.max(sqrt, 0))
        return [theta1a - atan2(d3, +theta1c), theta1a - atan2(d3, -theta1c)]
    }

    const theta2 = c => {
        const [theta1] = c
        const t1 =
            (Power(wx * Cos(theta1) + wy * Sin(theta1) - a1, 2) +
                Power(a2, 2) -
                Power(d4, 2) +
                Power(wz, 2)) /
            (2 * a2)
        const theta2a = atan2(wx * Cos(theta1) + wy * Sin(theta1) - a1, wz)
        const sqrt = Power(wx * Cos(theta1) + wy * Sin(theta1) - a1, 2) + wz * wz - t1 * t1
    ...
    const theta3 = c => {
        ...
        const atan_y = wx * Cos(theta1) + wy * Sin(theta1) - a1 - a2 * Cos(theta2)
        const atan_x = wz + a2 * Sin(theta2)
